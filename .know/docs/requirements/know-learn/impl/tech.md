...
| 场景 | 处理 |
|------|------|
| 对话无可提取知识 | 提示"未检测到高价值知识" |
| 同一对话多条知识 | 逐条处理，每条独立走完 Step 3-8 |
| 用户编辑摘要 | 重新跑冲突检测 |
| 用户改 tag | 调整详情文件结构匹配新 tag |
...
| 场景 | 输入 | 预期输出 |
|------|------|---------|
| 显式触发 | `/know learn` + 对话含技术选型 | 提取 rationale 类条目，展示确认 |
| 隐式信号检测 | 对话含 "must not"、"always" | 批量提议 constraint 条目 |
| 路由拦截 | 对话内容可通过 grep 直接获取 | `[skipped]` + 原因 |
| 冲突检测 | 新条目与已有条目 scope+tag 相同 | `[conflict]` + 4 选项 |
| 无信号 | 对话仅含日常问答 | "未检测到高价值知识" |
| 批量选择 | 检测到 3 条信号，用户选 "select" | 逐条处理选中项 |
